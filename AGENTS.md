...
- `README.md` intentionally repeats the homepage About and Why copy for repository visitors, but omits the website's Using section. Check those shared sections for drift whenever `content/index.md` changes; `pnpm verify` enforces this.
- Start machine-oriented discovery at `/llms.txt`, generated by `src/pages/llms.txt.ts`. It combines homepage context with direct links to every canonical case Markdown file; do not maintain those links by hand.
